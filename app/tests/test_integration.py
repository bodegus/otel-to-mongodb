...
class TestWritePersistenceIntegration:
    """Test write persistence scenarios."""

    @pytest.mark.integration
    @pytest.mark.requires_mongodb
    @pytest.mark.asyncio
    async def test_write_persistence_with_immediate_verification(
        self, otel_integration_context, json_traces_data
    ):
        """Test that data written can be immediately verified."""
        context = otel_integration_context

        traces_data = OTELTracesData(**json_traces_data["data"])
        await context.otel_service.process_traces(traces_data, request_id="persistence-test")

        # Immediately verify
        documents = await context.verify_telemetry_data(
            "traces", expected_count=1, request_id="persistence-test"
        )
        assert len(documents) == 1

        print("✅ Write persistence: Data immediately readable after write")

    @pytest.mark.integration
    @pytest.mark.requires_mongodb
    @pytest.mark.asyncio
    async def test_write_concern_durability(self, otel_integration_context, json_metrics_data):
        """Test that writes are durable."""
        context = otel_integration_context

        metrics_data = OTELMetricsData(**json_metrics_data["data"])
        await context.otel_service.process_metrics(metrics_data, request_id="durability-test")

        documents = await context.verify_telemetry_data(
            "metrics", expected_count=1, request_id="durability-test"
        )
        assert documents[0]["data_type"] == "metrics"

        print("✅ Write durability: Metrics data verified")


class TestTimeSeriesIntegration:
    """Integration tests for time series collections."""

    @pytest.mark.integration
    @pytest.mark.requires_mongodb
    @pytest.mark.asyncio
    async def test_timeseries_collections_created(self, otel_integration_context):
        """Verify time series collections are created with correct configuration."""
        context = otel_integration_context

        # Check that time series collections exist
        collections = context.db.list_collection_names()

        # Time series collections should have been created
        assert "traces" in collections or any("traces" in c for c in collections)
        assert "metrics" in collections or any("metrics" in c for c in collections)
        assert "logs" in collections or any("logs" in c for c in collections)
    ...
    @pytest.mark.requires_mongodb
    @pytest.mark.asyncio
    async def test_document_has_native_datetime_created_at(
        self, otel_integration_context, json_traces_data
    ):
        """Verify documents have native datetime created_at field for time series."""
        from datetime import datetime

        context = otel_integration_context

        traces_data = OTELTracesData(**json_traces_data["data"])
        await context.otel_service.process_traces(traces_data, request_id="ts-created-at-test")

        # Check document has created_at as native datetime
        docs = list(context.db["traces"].find({"request_id": "ts-created-at-test"}))
        assert len(docs) == 1

        doc = docs[0]
        # created_at should be native datetime for time series timeField
        assert "created_at" in doc
        assert isinstance(doc["created_at"], datetime)

        print("✅ Time series: Document has native datetime created_at")

    @pytest.mark.integration
    @pytest.mark.requires_mongodb
    @pytest.mark.asyncio
    async def test_timeseries_collection_options(self, otel_integration_context):
        """Verify time series collections have correct options configured."""
        context = otel_integration_context

        # Get collection info for metrics time series collection
        collections_info = context.db.list_collections(filter={"name": "metrics"})
        coll_list = list(collections_info)

        assert len(coll_list) == 1
        coll_info = coll_list[0]

        # Verify it's a time series collection with correct configuration
        assert "options" in coll_info
        options = coll_info["options"]
        assert "timeseries" in options
        ts_options = options["timeseries"]
        assert ts_options["timeField"] == "created_at"
        assert ts_options["granularity"] == "minutes"

        print(f"✅ Time series: Collection options validated: {ts_options}")

    @pytest.mark.integration
    @pytest.mark.requires_mongodb
    @pytest.mark.asyncio
    async def test_metrics_query_on_timeseries(self, otel_integration_context, json_metrics_data):
        """Test that metrics data can be queried from time series collection."""
        context = otel_integration_context

        metrics_data = OTELMetricsData(**json_metrics_data["data"])
        await context.otel_service.process_metrics(metrics_data, request_id="ts-query-test")

        # Query time series collection
        docs = list(context.db["metrics"].find({"request_id": "ts-query-test"}))
        assert len(docs) == 1

        # Verify OTEL data structure is preserved
        assert "resourceMetrics" in docs[0]
        assert docs[0]["data_type"] == "metrics"

        print("✅ Time series: Metrics data queryable from time series collection")

    @pytest.mark.integration
    @pytest.mark.requires_mongodb
    @pytest.mark.asyncio
    async def test_logs_written_to_timeseries(self, otel_integration_context, json_logs_data):
        """Test that logs data is written to time series collection."""
        context = otel_integration_context

        logs_data = OTELLogsData(**json_logs_data["data"])
        await context.otel_service.process_logs(logs_data, request_id="ts-logs-test")

        # Verify in time series collection
        docs = list(context.db["logs"].find({"request_id": "ts-logs-test"}))
        assert len(docs) == 1
        assert "created_at" in docs[0]
        assert "resourceLogs" in docs[0]

        print("✅ Time series: Logs data written to time series collection")

    @pytest.mark.integration
    @pytest.mark.requires_mongodb
    @pytest.mark.asyncio
    async def test_multiple_writes_to_timeseries(self, otel_integration_context, json_traces_data):
        """Test multiple writes to time series collection."""
        context = otel_integration_context

        # Write multiple documents
        for i in range(3):
            traces_data = OTELTracesData(**json_traces_data["data"])
            await context.otel_service.process_traces(traces_data, request_id=f"ts-multi-write-{i}")

        # Verify all documents in time series collection
        docs = list(context.db["traces"].find({"request_id": {"$regex": "^ts-multi-write-"}}))
        assert len(docs) == 3

        print("✅ Time series: Multiple writes succeeded")
